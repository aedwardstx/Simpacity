...
noids.each do |noid|
  [100,50,25,10,5,1].each do |percentile|
    Interface.all.each do |int|
      ...
      measure_sum = 0
      average = 0
      measurements = int.measurements.where(:noid => noid, :percentile => percentile, :collected_at => average_previous_hours.hours.ago..Time.now)
      ...
      #store to AR
      int.averages.where(:percentile => percentile, :noid => noid).destroy_all
      int.averages.create(:percentile => percentile, :noid => noid, :gauge => average)
      puts "done with ittr"
    end
    ...
    InterfaceGroup.all.each do |int_group|
      ...
      measure_sum = 0
      average = 0
      measurements = int_group.srlg_measurement.where(:noid => noid, :percentile => percentile, :collected_at => average_previous_hours.hours.ago..Time.now)
      measurements.each do |measures|
        measure_sum += measures.gauge
        measure_count += 1
      end
      ...
      #store to AR
      int_group.averages.where(:percentile => percentile, :noid => noid).destroy_all
      int_group.averages.create(:percentile => percentile, :noid => noid, :gauge => average)
